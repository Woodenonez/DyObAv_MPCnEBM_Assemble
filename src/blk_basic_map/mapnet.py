...
from blk_basic_map import basic_map
...
class SceneGraph:
    def __init__(self, scene:str, map_type:str, map_info:dict):
        ...
        if self.map_type == 'geometric':
            self.base_map = basic_map.GeometricMap(map_info['boundary'], map_info['obstacle_list'], map_info['inflation'])
        elif self.map_type == 'occupancy':
            self.base_map = basic_map.OccupancyMap(map_info['map_image'], map_info['threshold'])
        else:
            raise ValueError(f'Map type must be "geometric" or "occupancy", got {map_type}.')

        self.NG = NetGraph(return_map_network(scene)[0])
    ...
    def plot_path(self, ax:Axes, path, style='k--'):
    ...
    def plot_netgraph_edges(self, ax:Axes, edge_color='r'):
        nx.draw_networkx_edges(self.NG, nx.get_node_attributes(self.NG,'pos'), ax=ax, edge_color=edge_color)
