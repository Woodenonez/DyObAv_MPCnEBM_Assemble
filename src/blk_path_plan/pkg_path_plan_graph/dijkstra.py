...
class DijkstraPathPlanner:
    ...
    def __k_shortest_paths(self, source, target, k=1, weight_key='weight') -> Tuple[List[float], List[List[Any]]]:
        ...
        if source == target:
            return ([0], [[source]]) 
        G = self.G.copy() # self.NG is the original graph
    ...
    def k_shortest_paths(self, source, target, k=1, weight_key='weight', position_key='position') -> Tuple[List[float], List[PathNodeList]]:
        """
        Argument:
        ...
        for _path in _paths:
            ...
            for node_id in _path:
                ...
                path.append(PathNode(x, y, node_id))
        paths.append(path)
        return lengths, paths
