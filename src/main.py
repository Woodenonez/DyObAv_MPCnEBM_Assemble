...
traj_generator = MpcInterface(MPC_CFG, ROBOT_START_POINT, geo_map_rescale, external_base_path=base_path)
traj_generator.set_work_mode(mode='work')
...
for kt in range(MAX_SIM_TIME):
    ...
    print(f'The NN inference time is {round(nn_time/len(human_list),4)} per object.')
    ...
    start_time = timer()
    actions, pred_states, _, cost, the_obs_list, current_refs = traj_generator.run_step('work', full_dyn_obs_list, map_updated)
    mpc_time = timer() - start_time
    print(f'The MPC solver time is {round(mpc_time,4)}.')
    ...
    vel_ax.plot([0, (kt+1)*STIME], [traj_generator.traj_gen.base_speed, traj_generator.traj_gen.base_speed], 'r--')
    ...
    ### Finish-check: If humans finish their paths, assign new paths.
    for the_human in human_list:
