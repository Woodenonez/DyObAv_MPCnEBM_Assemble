...
from blk_mpc_planner.path_advisor.global_path_plan import GloablPathPlanner
from blk_mpc_planner.path_advisor.local_path_plan import LocalPathPlanner
...
class MpcInterface:
    def __init__(self, config_file_name:str, current_state:NumpyState, graph_for_mpc, external_base_path:PathNodeList, init_build:bool=False):
        ...
        current_node = PathNode(current_state[0], current_state[1], current_state[2], id=0) # dynamic node id-0
    ...
    def update_global_path(self, new_global_path:PathNodeList, current_node:PathNode=None):
        if current_node is None:
            current_node = PathNode(self.traj_gen.state[0], self.traj_gen.state[1], self.traj_gen.state[2], id=0)
        self.gpp = GloablPathPlanner(external_path=new_global_path)
        self.gpp.set_start_node(current_node) # must set the start point

    def update_local_path(self, new_graph_map=None):
