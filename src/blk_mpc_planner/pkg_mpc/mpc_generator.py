...
class MpcModule:
    ...
    def build(self, dynamics:Callable[..., cs.SX], use_tcp:bool=False):
        '''
        Description:
        ...
        init_obs_penalty = min(max(max(q_dyn), max(q_stc), 1), 100) # bounded in [1, 100]
        ...
        solver_config = og.config.SolverConfiguration() \
            .with_initial_penalty(init_obs_penalty) \
