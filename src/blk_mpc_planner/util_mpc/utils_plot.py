...
def plot_action(ax:Axes, action, ts, color='b'): # velocity or angular velocity
    time = np.linspace(0, ts*(len(action)), len(action))
    ax.plot(time, action, '-o', markersize = 4, linewidth=2, color=color)


def prepare_plot(fig:Figure, graph, start=None, end=None, legend_style='single', double_map=False, color_list=None, legend_list=None):
    """
    Prepare the plot.
...
def update_plot(axes:List[Axes], ts, xx, xy, vel, omega, cost, color):
